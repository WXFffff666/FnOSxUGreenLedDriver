...
> 版本：v4.0.0 — 回退LED核心逻辑+持久化登录+修Bug
...
- 一键全部常亮 / 全部自动 / 全部关闭
- **登录路由守卫**：未登录自动跳转登录页
- **颜色/亮度控制**：每LED颜色选择器+全局亮度滑块
- **跑马灯特效**：一键彩虹追逐模式
- **智能盘位**：所有盘可操作，空盘auto自动熄灯，默认auto模式
- **访问鉴权**：API 写入操作需 Token 验证，Token 自动嵌入前端无需手动配置
...
- **管理员登录**：单账户密码登录，仅管理员可控制，禁止多账户注册
- **热插拔检测**：每30秒自动检测新插入硬盘，槽位灯自动启用
...
# 生成 FnUGreenLed-2.0.0.x86_64.fpk
```
